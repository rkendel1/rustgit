...
fn postgres_migrations_apply_and_enforce_foreign_keys() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(store.migration_count()?, 7);
    ...
    assert!(store.table_exists("repository_context_snapshots")?);
    assert!(store.table_exists("repository_questions")?);
    assert!(store.table_exists("repository_answers")?);
    assert!(store.table_exists("repository_embeddings")?);
