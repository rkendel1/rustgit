...
| GoExecutionProvider | Go runtime provider | Implemented (stub process handle) | Registered in `WorkspaceManager::new`; impl in `src/lib.rs` | Router + execution graph cache key | `start` returns `pid_hint` with `go:` prefix | Go language + Go/Gin/Fiber/Echo frameworks | `go` | Returns healthy=true | Router fallback chain | Wired by default |
| PythonExecutionProvider | Python runtime provider (including uv projects) | Implemented (stub process handle) | Registered in `WorkspaceManager::new`; impl in `src/lib.rs` | Router + execution graph cache key | `start` returns `pid_hint` with `python:` prefix | Python language + Python web frameworks | pip/pipenv/poetry/uv | Returns healthy=true | Router fallback chain | Wired by default |
| JavaExecutionProvider | Java runtime provider | Implemented (stub process handle) | Registered in `WorkspaceManager::new`; impl in `src/lib.rs` | Router + execution graph cache key | `start` returns `pid_hint` with `java:` prefix | Java runtime spec and Maven/Gradle/pom/build.gradle signals | maven/gradle | Returns healthy=true | Router fallback chain | Wired by default |
...
| uv | Implemented | Detected/inferred in repository analysis + command synthesis + PythonExecutionProvider support for `uv` package manager (`src/lib.rs`) |
...
| go | Implemented | Go detected/inferred and handled by dedicated `GoExecutionProvider` in workspace runtime (`src/analyze/runtime_detector.rs:17`, `src/analyze/registry.rs:55-63`, `src/lib.rs`) |
| java | Implemented | Java detected in analyze and handled by dedicated `JavaExecutionProvider` in workspace runtime (`src/analyze/runtime_detector.rs:18`, `src/analyze/framework_detector.rs:68-75`, `src/lib.rs`) |
